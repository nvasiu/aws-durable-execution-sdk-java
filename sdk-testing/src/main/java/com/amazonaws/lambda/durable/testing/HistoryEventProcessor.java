...
public class HistoryEventProcessor {
    ...
    public <O> TestResult<O> processEvents(List<Event> events, Class<O> outputType) {
        ...
        for (var event : events) {
            ...
            if (operationId != null) {
                operationEvents.computeIfAbsent(operationId, k -> new ArrayList<>()).add(event);
            }
            ...
            if ("ExecutionSucceeded".equals(eventType)) {
                ...
                if (details != null && details.result() != null && details.result().payload() != null) {
            ...
            if ("StepStarted".equals(eventType) && operationId != null) {
                operations.putIfAbsent(operationId, createStepOperation(operationId, event.name(), null, OperationStatus.STARTED, 1));
            } else if ("StepSucceeded".equals(eventType) && operationId != null) {
                var details = event.stepSucceededDetails();
                var stepResult = details != null && details.result() != null ? details.result().payload() : null;
                var attempt = details != null && details.retryDetails() != null ? details.retryDetails().currentAttempt() : 1;
                operations.put(operationId, createStepOperation(operationId, event.name(), stepResult, OperationStatus.SUCCEEDED, attempt));
            } else if ("StepFailed".equals(eventType) && operationId != null) {
                var details = event.stepFailedDetails();
                var attempt = details != null && details.retryDetails() != null ? details.retryDetails().currentAttempt() : 1;
                operations.put(operationId, createStepOperation(operationId, event.name(), null, OperationStatus.FAILED, attempt));
            }
            
            // Process wait events
            if ("WaitStarted".equals(eventType) && operationId != null) {
                operations.putIfAbsent(operationId, createWaitOperation(operationId, event.name(), OperationStatus.STARTED, event));
            } else if ("WaitSucceeded".equals(eventType) && operationId != null) {
                operations.put(operationId, createWaitOperation(operationId, event.name(), OperationStatus.SUCCEEDED, event));
            } else if ("WaitCancelled".equals(eventType) && operationId != null) {
                operations.put(operationId, createWaitOperation(operationId, event.name(), OperationStatus.CANCELLED, event));
            }
        }
