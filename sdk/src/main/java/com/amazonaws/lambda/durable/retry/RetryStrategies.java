 ...
 * 
 * This class provides preset retry strategies for common use cases,
 * as well as factory methods for creating custom retry strategies with
...
public class RetryStrategies {
     ...
     * baseDelay = min(initialDelay × backoffRate^attemptNumber, maxDelay)
     * 
    ...
    public static RetryStrategy exponentialBackoff(
        ...
        return (error, attemptNumber) -> {
            ...
            double baseDelay = Math.min(
                    initialDelaySeconds * Math.pow(backoffRate, attemptNumber),
                    maxDelaySeconds);

            // Apply jitter
            double delayWithJitter = switch (jitter) {
                case NONE -> baseDelay;
    ...
    /**
     * Creates a simple retry strategy that retries a fixed number of times with a
     * fixed delay.
     * 
     * @param maxAttempts Maximum number of attempts (including initial attempt)
