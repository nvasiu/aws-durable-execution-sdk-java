...
public class RetryInProcessExample extends DurableHandler<Object, String> {
    ...
    protected String handleRequest(Object input, DurableContext context) {
        ...
        DurableFuture<String> asyncStep = context.stepAsync("flaky-async-operation", String.class, () -> {
            int attempt = attemptCount.incrementAndGet();
            logger.info("Async operation attempt #{} in thread: {}", attempt, Thread.currentThread().getName());
        ...
        String syncResult = context.step("long-running-operation", String.class, () -> {
            logger.info("Starting long-running operation (10 seconds) in thread: {}", Thread.currentThread().getName());
            try {
